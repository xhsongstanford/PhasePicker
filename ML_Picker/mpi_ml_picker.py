...
def get_amplitudes(stream_amp, picks_df ,label):
    ampls_z = []
    ampls_n = []
    ampls_e = []
    nois_z = []
...
def get_snrs(stream0, picks_df, label):
    ...
    for i in range(len(picks_df)):
        ...
        for j in range(len(stream0)):
            ...
            if picks_df['phase'][i] == 'P':
                ...
                nos_end = int(round((pick_time - tr.meta.starttime)/dt))-int(2/dt)
            else:
                if label == 'skynet':
                    sgn_start = int(round((pick_time - tr.meta.starttime)/dt))
                    sgn_end = int(round((pick_time - tr.meta.starttime)/dt))+int(4/dt)
                else:
                    sgn_start = int(round((pick_time - tr.meta.starttime)/dt))
                    sgn_end = int(round((pick_time - tr.meta.starttime)/dt))+int(3/dt)
                nos_start = int(round((pick_time - tr.meta.starttime)/dt))-int(6/dt)
                nos_end   = int(round((pick_time - tr.meta.starttime)/dt))-int(2/dt)
            if tr.meta.channel[-1] == 'Z':
                try:
...
def run_and_save_picks(stream, stream2, stream_amp, model, SaveDir = 'Picks', label = 'diting'):
    ...
    ampls_z, ampls_n, ampls_e, nois_z, nois_n, nois_e, rms_z, rms_n, rms_e = get_amplitudes(stream_amp, picks_df, label)

    snrs_z, snrs_n, snrs_e, warnings= get_snrs(stream, picks_df, label)
    
    if len(ampls_z) != len(picks_df):
        print(f'Rank {rank}: **** station error {sta} on {crr_date.date}*******')
...
def process_single_day(curr_date, SaveDir):
    ...
    for net in nets:
        ...
        for sta_path in sta_paths:
            ...
            if stream_amp[0].meta.delta < 0.05:
            ...
            for i in process_idx:
                ...
                if labels[i] in ['diting']:
                    ...
                    try:
                        signal.alarm(300)
                        run_and_save_picks(stream_high, stream_low, stream_amp, model=models[i], SaveDir=SaveDir, label=labels[i])
                    ...
                    try:
                        signal.alarm(300)
                        if net == 'AM':
                            run_and_save_picks(stream_high, None, stream_amp, model=models[i], SaveDir=SaveDir, label=labels[i])
                        else:
                            run_and_save_picks(stream, None, stream_amp, model=models[i], SaveDir=SaveDir, label=labels[i])
